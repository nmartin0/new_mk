	...
	mach_port_t		name)
{
	...
	new_sp = (vm_offset_t)new_argv;
	__asm__ volatile(
		/* AI-ONLY NOTE: *new_argv moved to outputs, as above. The
		 * asm stores into it ("movl %%edx, ..."), but it was
		 * declared an input, so GCC was free to hold it in a
		 * register and discard the store -- leaving the argument
		 * off the new stack. Operands renumbered: outputs first. */
		"movl %1, %%edx; movl %%edx, %0"
		: "=g" (*new_argv)
		: "g" (name)
		: "%edx");
	/*
