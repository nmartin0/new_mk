...
# (osfmk/{src,rc_files,link,tools}) and a sandboxrc.template.
#
...
#
# 2. osfmk/src/makedefs is an incomplete ODE installation. Its
...
OSFMK="${REPO_ROOT}/osfmk"

[ -d "${OSFMK}/src" ] || {
	echo "no osfmk/src -- has the vendor import been committed?" >&2
	exit 1
}
...
# is modified. Generated rather than committed so that it never
# appears in git diff <vendor-import>..HEAD -- osfmk/.
...
# --- 4. the sandboxrc --------------------------------------------------
# Derived from build/sandboxrc.template, with the base pointing at
