...
# NO COMPILER-SUPPLIED FILE IS USED, by design. This used to add
# gcc's own include directory with -isystem and link its libgcc.a:
#
#	GI=$(gcc -m32 -print-file-name=include)
#	LG=$(gcc -m32 -print-libgcc-file-name)
#
# Both are gone. libgcc was needed only for __divdi3, __udivdi3,
# __moddi3 and __umoddi3, the 64-bit division runtime, which the tree
# now builds itself from libkern/qdivrem.c -- shipped with LITES and
# never compiled -- plus the four wrappers in libkern/quaddiv.c.
#
# gcc's include directory was needed only for <stdarg.h>, reached
# through OSFMK's own mach.h. The canonical place for it is OSFMK's
# standalone include set, sa_mach, appended to the include path.
#
# This is upstream's own pattern, not an invention. conf/Makerules
# builds INCDIRS as
#
...
# -- so sa_mach goes on the end in the same way. Because it is LAST it
# cannot shadow anything: LITES's string.h, sys/ and i386/ still win,
# and only names LITES does not provide at all, which is stdarg.h,
# fall through to Mach.
#
# sa_mach is the right set for this program specifically. It is the
# standalone Mach environment, meant for programs that link
# libmach_sa, which the LITES server and emulator do.
#
# Two earlier attempts here are worth not repeating: a hand-written
# forwarding header pointing at <machine/stdarg.h>, which satisfied a
# Mach include with a LITES header; and one pointing at
# sa_mach/stdarg.h, which needed two further forwarders and still lost
# to obj/include on the path. Appending the directory needs neither.
